...
- **`[slug]/page.tsx`에서 새 도구는 서로 독립적인 *두 개의* if/else 체인에 각각 배선해야 한다 — 하나(`ToolBody`의 콘텐츠 분기)만 추가하고 `generateMetadata`의 분기를 빠뜨려도 에러 없이 조용히 실패한다.** barcode-generator 실측: platform-engineer가 "라우트 배선 완료"를 보고했고 실제로 `ToolBody`(StructuredData+메인+HowTo+Faq 렌더)는 정확히 배선됐지만, `generateMetadata`의 `else if (slug === …)` 체인엔 `barcode-generator`를 안 넣어 마지막 `else { return {}; }`로 떨어졌다 — Next.js는 빈 metadata를 부모 레이아웃 기본값으로 조용히 채우므로 페이지가 **"Jurepi — Free Online Tools"라는 사이트 기본 제목 + canonical/hreflang 없음**으로 렌더됐다(빌드·전체 유닛·E2E 전부 그린, 리더가 프리렌더 HTML `<title>`을 직접 grep해서야 적발). **또한 이 라우트는 모든 도구의 H1/eyebrow/lead를 이미 `ToolContent`가 공유 `<ToolIntro>`(`tools.<slug>.intro.*` 소비)로 한 번 렌더한다 — 새 도구가 자체 `XxxIntro.tsx`를 만들어 같은 `intro.*` 키를 또 렌더하면 H1이 페이지에 두 번 나온다**(barcode-generator에서 실제로 만들었다가 적발·삭제). **예방:** ① 새 도구 라우트 작업은 항상 **① `ToolBody`의 콘텐츠 분기 ② `generateMetadata`의 메타 분기** 두 곳 모두에 슬러그를 추가했는지 grep으로 대조(`grep -c "slug === '<slug>'"` 이 파일에서 정확히 2여야 함) ② 리더는 "라우트 배선 완료" 보고를 믿지 말고 배포/빌드 후 프리렌더 HTML의 `<title>`이 사이트 기본값이 아닌지 직접 확인 ③ 새 도구는 자체 Intro 컴포넌트를 만들지 말 것 — H1/eyebrow/lead는 이미 `ToolIntro`가 라우트 레벨에서 전 도구 공통으로 소유한다(StructuredData/HowTo/Faq와 마찬가지로 도구 컴포넌트는 인터랙티브 본체만 렌더).
