...
- **공유 헤더/셸은 320px 가로 예산이 빡빡하다 — 헤더에 요소(브랜드 링크·컨트롤·검색)를 *추가*하면 우측 컨트롤 클러스터가 320에서 넘칠 수 있다.** 실측: 헤더 좌(워드마크+"AI" 링크)+우(검색·로케일·테마 `gap-2`) 클러스터 합이 320px에서 `nav` 콘텐츠 박스(px-3=296px)를 24px 초과해 `scrollWidth` 332가 됐다(AI 링크가 2026-07-20 추가된 뒤 누적). 이 오버플로는 **모든 페이지의 공유 헤더**에 있지만 320px no-overflow 단언이 있는 일부 도구 스펙(8개)에서만 드러나 특정 도구 결함으로 위장한다(→ 귀속법은 `integration-qa`). 완화 레버(안전순): `nav px-2`(base만, `sm:px-6` 유지 — 데스크톱 정렬 스펙 불변) · 우측 클러스터 `gap-1 sm:gap-2` · 링크의 장식 아이콘(외부-링크 화살표 등) `hidden sm:inline-block`. 헤더/셸을 건드리는 변경은 완료 전 **320px에서 `nav` 자식들의 우측 엣지를 측정**(`right ≤ 뷰포트`)하라 — 헤더 at-rest DOM을 안 바꾸는 변경(예: 드롭다운에 안 보이는 prop 추가)이라도 기존 오버플로가 게이트에 걸리면 함께 처리 여부를 사용자에게 확인.
...
- [ ] **배경을 지정하는 요소(`<mark>` 하이라이트·pill·배지·칩)에 텍스트 색을 *명시*했는가 — 배경만 주면 상속/UA 기본 글씨색이 다크모드에서 비가시가 된다.** 헤더 검색 하이라이트 실측: `<mark className="bg-accent-sun-soft/50 font-semibold">`가 **텍스트 색 미지정** → 브라우저 `<mark>` 기본색(거의 검정)이 적용돼 라이트에선 약하고(반투명 배경까지 겹쳐) **다크에선 검정 글씨가 어두운 배경에 묻혀 매칭 글자가 안 보였다**(tsc·유닛·빌드·color-tokens 전부 그린 — 라이트/다크 라이브 스크린샷으로만 적발). `<mark>`는 UA 기본 배경(노랑)+글씨(검정)를 갖는데 배경만 오버라이드하면 글씨는 검정으로 남는다. 규칙: 배경을 칠하는 인라인 강조/pill은 **design system soft+ink 쌍**으로(`bg-accent-<c>-soft text-accent-<c>-ink`, `popular` Badge와 동일 패턴) — soft+ink는 라이트/다크 3매체 모두 대비가 보장된다. 반투명 `/50`으로 배경을 흐리지 말고 풀 불투명 soft를 쓴다. 검증 게이트: 하이라이트/pill이 있는 화면을 **라이트·다크 둘 다** 열어 매칭 글자가 또렷한지 확인(프레시 로드만이 아니라 위 "다크모드 라이브 토글" 절과 짝).
...
- [ ] **인라인 스타일/서드파티 SDK 주입 HTML의 `var(--토큰)` 문자열도 팬텀 검사를 했는가** — Tailwind 클래스 가드(`color-tokens.test`)는 인라인 `style="background-color: var(--accent)"` 같은 CSS 변수 참조를 **스캔하지 않는다**. 미존재 변수는 조용히 빈 값=투명 렌더(restaurant-map: NAVER 지도 마커 배경이 팬텀 `var(--accent)`로 투명 → 마커가 사실상 안 보여 사용자가 "지도 무응답"으로 인식; tsc·유닛·빌드·E2E 전부 그린). 액센트 변수의 실명은 `--accent-{coral,mint,sky,sun,grape,rose}`(카테고리별)이지 `--accent`가 아니다. 반환 전 변경 파일의 `var(--` 를 grep해 `globals.css` 정의와 대조하고, 지도/캔버스/SDK 주입 표면은 렌더된 화면에서 색이 실제 보이는지 확인한다(`getComputedStyle(...).backgroundColor`가 `rgba(0,0,0,0)`이면 팬텀). **자동화(2026-07-26): `scripts/validate-color-tokens.mjs`가 이제 인라인 `var(--토큰)`도 스캔해 `src/**/*.css`(tokens.css+globals.css) 정의와 대조한다** — 정적 리터럴 `var(--foo)`가 미정의면 실패(동적 `var(--accent-${c})` 템플릿은 정적 해석 불가라 스킵). 이 가드는 `color-tokens.test`(전역 vitest)에 더해 **`prebuild`에서도 실행**되므로, 스코프 vitest로 우회해도 `pnpm build`(=CF 배포)가 팬텀 클래스/`var()`에서 실패한다 — 수동 grep은 이제 조기 신호일 뿐, 하드 게이트는 빌드가 강제한다.
...
- [ ] **커스텀 `role="tablist"`/`role="tab"`을 새로 만들 때 롤링 탭인덱스(선택 탭만 `tabIndex=0`, 나머지 `-1`)와 화살표 키의 실제 DOM 포커스 이동을 구현했는가 — `aria-selected`만 갱신하고 포커스를 안 옮기면 키보드 포커스 링이 방금 떠난 탭에 방치된다(WAI-ARIA APG Tabs 패턴 위반).** barcode-generator FormatSelector 실측: 화살표 키 핸들러가 `onChange(newFormat)`만 호출하고 `.focus()`는 호출하지 않아, 4개 탭 전부 `tabIndex=0`(개별 Tab 스톱)인 채 선택 상태만 바뀌었다(tsc·유닛·빌드 전부 그린 — 라이브 DOM `tabIndex`/`document.activeElement` 실측으로만 적발). 구현: 탭마다 ref 배열을 두고, 새 인덱스로 `onChange` 호출 직후 `tabRefs.current[newIndex]?.focus()`(선택 프롭이 아직 리렌더 전이라 그 탭이 `tabIndex=-1`이어도 스크립트 `.focus()`는 항상 허용된다). 회귀 테스트: 비선택 탭들의 `tabIndex==='-1'` 단언 + 화살표 키 입력 후 새 탭이 `toHaveFocus()`인지 단언(`aria-selected` 값만 보면 포커스 이동 누락을 놓친다).
