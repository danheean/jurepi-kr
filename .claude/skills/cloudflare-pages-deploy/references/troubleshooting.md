...
`next start`로 본 결과는 CF와 다르다(헤더/리다이렉트 파일 미적용). 반드시 `wrangler dev`(Workers 정적 에셋) / `wrangler pages dev out`(Pages) 또는 실제 배포로 본다.

---

## 0. (Workers 경로) 배포가 OpenNext SSR로 자동 전환 / `WORKER_SELF_REFERENCE` 실패

**증상:** CF 빌드 로그에 `@opennextjs/cloudflare migrate`, 수백 개 패키지 설치, `.open-next/worker.js`가 보이고 끝에 `Service binding 'WORKER_SELF_REFERENCE' references Worker '<name>' which was not found [code 10143]`.

**원인:** 배포 명령 `npx wrangler deploy`가 repo에 wrangler 설정이 **없을 때** Next.js를 감지해 OpenNext(풀 SSR Worker)로 자동 마이그레이션. self-ref 바인딩 이름(package.json `name`)이 CF 워커명과 달라 실패. 순수 SSG엔 불필요.

**해결:** repo 루트에 `wrangler.jsonc`(assets-only: `assets.directory=./out`, `main` 없음, `name`=CF 워커명) 커밋 → `wrangler deploy`가 정적 에셋만 올림(OpenNext 건너뜀). SKILL.md "채택된 실제 배포 경로" 참조.

---

## 0b. `output:'export'` 빌드가 metadata 라우트에서 실패

**증상:** `Error: export const dynamic = "force-static"/export const revalidate not configured on route "/manifest.webmanifest"`(또는 `/sitemap.xml`, `/robots.txt`) `with "output: export"`.

**원인:** route handler(`manifest`/`sitemap`/`robots`)가 정적 익스포트에서 정적임을 명시해야 함.

**해결:** `src/app/{manifest,sitemap,robots}.ts` 각각에 `export const dynamic = 'force-static';` 추가.

---

## 0c. `NEXT_PUBLIC_*`가 프로덕션에서 빈 값 (sitemap 도메인·마스코트 링크 등)

**증상:** 로컬은 정상인데 배포본 sitemap/canonical이 잘못된 기본 도메인, 마스코트/외부 링크가 비어 있음.

**원인:** 값이 `.env.local`(gitignored)에만 있어 CF 빌드가 못 봄(5번의 변형 — `.env.local`은 클론되지 않는다).

**해결:** 비밀 아닌 `NEXT_PUBLIC_*`를 `.env.production`(커밋)에 두거나 CF 대시보드 빌드 env에 등록 후 **재빌드**. 빌드 후 `grep -r "<값>" out/`로 인라인 확인.
