...
- **도구를 처음으로 `main`에 병합·배포할 때(기존 라이브 도구의 콘텐츠 변경이 아니라 라우트 자체가 신규)는 그 라우트가 "404 → 200"으로 전환되는 것을 신호로 삼고, 대형 병합(90+ 파일 등)은 평소(~140~180초) 폴링 창보다 훨씬 오래 걸릴 수 있음을 감안한다.** charades 최초 병합 배포 실측: push 직후 ~7분이 지나도 `/ko/tools/charades`가 계속 404였다 — 동시에 홈(`/ko`)·기존 도구(`/ko/tools/speed-quiz`)는 계속 200을 반환해 **사이트 전체 장애가 아님을 확인**했고(다른 라우트가 살아있으면 배포 파이프라인이 아직 새 빌드를 만들고 있는 것이지 실패가 아니다), 이어서 200으로 전환됐다. **판정 규율:** ① 새 라우트는 404 사라짐이 1차 신호(기존 라우트의 "새 문자열 등장"과 다른 형태) ② 다른 라이브 라우트가 여전히 200이면 조급하게 "배포 실패"로 결론짓지 말고 대기 지속(대형 병합=긴 CF 빌드는 정상 범위) ③ 200 전환 후에도 클라이언트 렌더 콘텐츠(예: 인터랙티브 덱 그리드)는 `curl`의 원시 HTML에 안 잡힐 수 있으니(아래 항목 참고) 실제 브라우저/Playwright 스크린샷으로 최종 확인한다.
- **`curl`의 원시 HTML은 SSR된 SEO 섹션(Intro/HowTo/FAQ/JSON-LD)만 보여주고, 순수 클라이언트 렌더 영역(예: 인터랙티브 덱 브라우저 그리드)은 하이드레이션 후에만 나타나 원시 HTML엔 전혀 없다 — 이건 결함이 아니라 설계대로다.** charades 실측: 배포 직후 `curl`로 받은 HTML(311KB)엔 `SoftwareApplication` JSON-LD는 1개 정상 존재했지만 `deck-card-*`/`일상 동작` 같은 덱 그리드 텍스트는 0건이었다(그 부분은 인터랙티브 SPA 영역이라 mounted 게이트 안에서만 렌더). Playwright로 실제 브라우저 렌더를 스크린샷해서만 덱 그리드 정상 노출을 확인할 수 있었다. **판정:** SEO 섹션 검증=curl grep, 인터랙티브 UI 검증=브라우저 스크린샷 — 도구 성격에 맞는 검증 도구를 고른다(둘 다 grep 0이라고 배포 실패로 오판하지 말 것).
