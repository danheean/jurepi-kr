...
7. **커버리지 래칫 + 배포 차단 게이트 (2026-07-26 — 탐지를 예방으로).** 위 "71.79% 드리프트"는 그새 회복됐다: 측정 **80.97%**(statements)/86.56%(branches)/84.79%(functions). 이제 `vitest.config.ts` `coverage.thresholds`에 하드 플로어(statements/lines **79** · branches **84** · functions **82**, 측정치보다 ~2pt 낮게)를 걸어 **`pnpm test:coverage`가 하한 미달 시 exit 1** — 72% 플래토로의 조용한 재드리프트가 불가능하다. 불활성/타-게이트 검증 파일(type-only·barrel·`*StructuredData.tsx`[프리렌더 검증]·`analytics`[E2E 검증])은 사유 주석과 함께 coverage 스코프에서 제외(정직화). **또한 팬텀 토큰·i18n 드리프트는 이제 `prebuild`(=CF 배포 시 `pnpm build`가 자동 실행)에서 하드 게이트한다:** `scripts/validate-color-tokens.mjs`(인라인 `var(--)`까지) + `scripts/validate-i18n.mjs`(ko↔en 키 패리티 + 정적 `t()` 키가 카탈로그에 존재). 스코프 vitest로 우회해도 **배포 빌드가 실패**한다(둘 다 `src/__test__/{color-tokens,i18n-keys}.test.ts`로 전역 vitest에도 이중 등록). i18n 검사는 오탐 0을 위해 동적 네임스페이스(`getTranslations({namespace:\`tools.${slug}\`})`) 파일과 opposite-language 누수(브랜드명·ICU·한자 어원 노이즈)는 게이트에서 제외 — 후자는 여전히 리더 수동 검토.
...
- **Playwright 기본(병렬 워커) 실행이 특정 스펙 전체(5/5)를 `page.goto` 타임아웃으로 전멸시키면, 코드 결함으로 단정하기 전에 호스트 리소스 경합부터 배제하라.** charades 병합 세션 실측: 직전에 570초짜리 전체 vitest + 수 분짜리 `pnpm build`를 돌린 직후 같은 머신에서 병렬 워커로 charades E2E를 돌리자 5개 시나리오 전부 `page.goto: Test timeout of 30000ms exceeded`로 실패(서버는 `curl`로 200 확인됨 — 서버는 살아있는데 브라우저 네비게이션만 멎음). **진단:** 완전히 무관한, 이미 검증된 기존 스펙(예: `age-calculator.spec.ts`)을 같은 조건(병렬 워커)으로 함께 돌려본다 — 그것도 간헐 실패하면(실측: 9/11 통과, 2건 같은 시그니처로 실패) 내 변경과 무관한 **환경 리소스 경합**이 확정이다. **해결:** `npx playwright test <spec> --workers=1`로 재실행 — 직렬화하면 브라우저 인스턴스 간 CPU 경합이 사라져 진짜 결과(정상: 5/5 통과)가 나온다. 병렬 워커 전멸을 "회귀"로 오판해 코드를 뒤지지 말 것 — 먼저 무관 스펙 컨트롤 + `--workers=1` 재시도로 환경 요인을 분리한다.
...
### 다크모드 검증은 "프레시 로드"와 "라이브 토글" 둘 다 — 하나만 보면 놓친다 (2026-07-21)

**다크모드 색상 검증을 `localStorage`에 테마를 미리 심고 새로고침한 상태로만 하면, 라이브로 토글 버튼을 눌렀을 때만 재현되는 진짜 버그를 놓친다.** 로또 생성기 실측: `text-text` 등 토큰 기반 색 클래스를 쓴 링크가 **프레시 로드**(테마를 미리 설정하고 새로고침)에서는 완벽했지만(16.56:1), 헤더의 테마 **토글 버튼을 클릭**하면 `data-theme`는 정확히 바뀌는데 그 요소의 `color`가 **토글 이전 값에 영구 고정**됐다(1초는커녕 12초를 기다려도 안 풀림) — 실제로는 `transition-colors`가 걸린 요소가 커스텀 프로퍼티 변경에 반응해 트랜지션은 시작하지만 완료되지 않는 브라우저 렌더링 특성이었다(design-system-fidelity에 근본 원인·처방 인코딩). 처음엔 "트랜지션 타이밍 아티팩트"로 오판하고 넘어갈 뻔했다 — **1초 기다려도 안 바뀌면 그건 타이밍이 아니라 진짜 버그**라는 신호다.

- **검증 순서:** ① 프레시 로드(테마를 미리 심고 새로고침) — 이건 "정적으로 맞다"만 증명한다. ② **반드시 추가로** 같은 페이지에서 헤더 토글 버튼을 실제로 클릭 → 1초 이상 대기 → 같은 요소의 색이 `data-theme`와 일치하는지 재확인. 둘 다 통과해야 "다크모드 지원"으로 인정한다.
- **판별:** 값이 안 바뀌는데 "트랜지션 시간이 더 필요한가 보다"라고 넘기지 말고, `el.style.transition='none'`을 강제해 **즉시** 올바른 값으로 스냅되는지 실험하라 — 스냅되면 트랜지션-고착 버그 확정(design-system-fidelity 처방으로 반송), 안 스냅되면 다른 원인(CSS 특이도·캐스케이드)을 계속 판다.
- **전체 페이지 계약 대비 스캔이 스팟체크보다 실제로 더 많이 잡는다.** 이번 세션에서 개별 요소를 눈으로 훑을 땐 5개 요소 정도만 의심했지만, 페이지의 모든 텍스트 보유 요소를 순회해 전경/배경 대비를 계산하는 스크립트를 돌리자 **문서화된 디자인 토큰 자체의 결함**(`--text-muted`가 `--surface-sunken` 배경에서만 4.23:1로 미달 — 다른 배경에선 통과라 안 걸렸던 것)과 **공유 컴포넌트의 팬텀류 색 사용**(`Markdown.tsx`의 `text-accent-mint`가 원색 그대로 텍스트에 — 1.73:1)까지 걸렸다. 새 도구/컴포넌트의 다크모드 감사는 눈으로 3~4곳 훑기보다, 렌더된 페이지의 텍스트 보유 요소를 전수 순회해 전경/배경 대비를 계산하는 스크립트(라이트/다크 각각)를 우선한다.

### 공유 토큰·공유 컴포넌트 수정은 전체 프로젝트 테스트 스위트로 검증한다 (2026-07-21)

`src/styles/tokens.css`의 색 토큰이나 `src/components/markdown/` 같은 여러 도구가 공유하는 컴포넌트를 고칠 땐, 그 도구 하나의 스코프 테스트(`vitest run src/components/tools/<slug>`)만으로는 회귀를 증명하지 못한다 — 영향 범위가 수십~수백 파일이기 때문이다. 실측: `--text-muted` 토큰 변경이 `src/components/tools/` 아래 **51개 파일**에서 쓰이는 조합에 영향을 줬고, `Markdown.tsx`의 인라인 코드 색 변경은 마크다운을 렌더하는 **모든** 도구에 영향을 준다. **예방:** 공유 파일(토큰·`components/markdown/`·`components/ui/`·`lib/` 공용 유틸)을 수정하면 반드시 `npx vitest run src`(전체, 스코프 없이)와 `tsc --noEmit`을 프로젝트 루트에서 돌리고, 그 수치(파일 수·테스트 수)를 보고에 인용한다. 이번 세션 실측 수치: 6,143개 테스트 전체 통과, 0 회귀 — 이 정도 근거 없이는 "안전하다"고 말할 수 없다.
...
- **320px 오버플로 실패가 *직렬 재실행(`--workers=1`)에서도 재현되면 플래키가 아니라 진짜다(공유 셸 회귀 or 선존) — offending 요소를 측정하고 `main` 자체 빌드 출력과 대조해 귀속하라.** (2026-07-26 이전 로그가 정확히 이 8건[base64·charades·cheer·cron·knitting·tool-characters·unit-converter·url-encoder]을 "선존 플래키·직렬 41/41 통과"로 오귀속한 것을 정정: 이 세션에서 직렬 재실행도 같은 8건이 결정론적으로 실패했다.) 병렬 부하 플래키(위 항목: 단독 그린·재실행마다 대상 이동)와 달리, **직렬에서도 같은 스펙이 안정적으로 실패하면** 코드/레이아웃 결함이다. 귀속 절차: ① 실패 페이지를 320px로 열어 `document.querySelectorAll('*')`를 순회해 **가장 오른쪽 엣지 요소**를 찾고 그것이 `header.contains(el)`인지(=공유 셸) 측정 ② **공유 셸이 원인이면 회귀 vs 선존을 `main` 자신의 빌드로 판별** — `main` 워크트리에 이미 `out/`이 있으면(또는 새로 빌드) `serve out -l <포트>`로 띄우고 같은 320px 측정을 돌려, **main도 같은 오버플로면 선존**(내 변경과 무관 — 특히 내 diff가 헤더 at-rest DOM을 안 바꿨으면 논리적으로도 선존), 내 브랜치만 넘치면 회귀. ③ **공유 헤더/셸 오버플로는 320px no-overflow 단언이 있는 일부 도구 스펙에만 게이트가 있어 "특정 도구 결함"으로 위장한다** — 미접촉 도구 8종이 동시에 320px만 실패하고 offending 요소가 전부 `header` 내부면 공유 셸 회귀 신호다. 선존이면 사용자에게 보고하고 처리 방침을 확인(수정 시 완화 레버는 design-system-fidelity의 "공유 헤더 320px 예산" 절).
