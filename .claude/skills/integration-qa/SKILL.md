...
5. **SNS 공유 버튼 존재.** 모든 도구 페이지·엔티티 스포크 페이지는 `<ShareButtons />`(`@/components/share`)를 렌더해야 한다 — `[slug]/page.tsx` 템플릿 배선이라 레지스트리 도구는 자동이지만, **`[slug]` 밖 새 라우트 파일(스포크)은 직접 배선 필요**(누락 시 반송). 게이트: 프리렌더 HTML에서 공유 aria-label(예: "네이버로 공유") grep 또는 라이브에서 버튼 row 확인.
