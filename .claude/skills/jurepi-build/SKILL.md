...
- **오래된 베이스의 횡단 변경(폴리시·a11y·리팩터)을 병합할 때, origin/main이 그새 *같은 컴포넌트*를 발전시켰다면 충돌 해소만으로 끝이 아니다 — 상대 브랜치가 새로 추가한 형제 코드에 내 패턴을 수동 재적용한다.** cheer 실측: 내 `fun/cheer` 폴리시(스와치 색 단일소스·`aria-pressed`·터치타깃 `min-h-11`)는 autosize *이전* 코드 기반이었고, origin/main엔 이미 cheer-autosize가 `CheerControls.tsx`에 수동/자동 모드 토글·디바이스 타입 버튼을 추가해 둔 상태였다. `CheerControls.tsx` 텍스트 충돌 1건을 해소해도, **autosize가 들여온 새 버튼들(수동 SIZES·디바이스 타입)엔 내 `aria-pressed`·`min-h-11`이 자동으로 붙지 않는다**(git 3-way 병합은 내 diff를 그들의 새 라인에 전파하지 않음 — 32px·상태 미노출 그대로). 규율: 충돌 해소 후 **병합본 전체에서 내 횡단 규칙을 다시 grep**(`min-h-11`/`aria-pressed` 커버리지, 팬텀 토큰, `focus-visible:`)해 상대가 추가한 형제 요소까지 일관되게 적용하고, 라이브 계측(예: 최소 컨트롤 높이 44px·`aria-pressed` 개수)으로 확인한다. 두 기능이 한 컴포넌트에서 공존하므로 병합본은 **양쪽 테스트 세트가 함께 그린**(cheer: a11y 테스트 + autosize 테스트 = CheerControls.test 21건)이어야 한다.
- **병합 직전 `git status --short`가 `main` 워크트리에서 동시 진행 중인 다른 세션의 미커밋 변경을 보여주면(WIP 파일이 계속 늘어나는 등), 특히 내 병합 대상과 같은 공유 파일(`i18n/messages/*.json`·레지스트리 등)을 그 세션도 건드리고 있다면 자동 병합을 강행하지 말고 사용자에게 확인한다.** cheer-autosize 세션 실제: `main`에 로또 생성기 관련 다른 세션이 활성 편집 중(`SettingsPanel.tsx`·`useLottoGenerator.ts`·`en.json`·`ko.json`이 계속 늘어남)이었고, 내 커밋도 같은 `en.json`/`ko.json`을 건드려 병합 시 그 세션의 미저장 작업과 충돌할 위험이 있었다. `git merge --ff-only`는 겹치는 미커밋 변경을 감지하면 안전하게 거부하긴 하지만(데이터 손실 없음), 시도 자체가 다른 세션의 워크플로를 방해할 수 있어 **사용자에게 병합 보류 여부를 먼저 확인**했다(`AskUserQuestion`). 겹치는 공유 파일이 없다면(예: 정적 에셋 하나만 건드리는 별도 커밋) 그 부분만 분리해 먼저 커밋·push해도 된다 — 충돌은 파일 단위로 판단한다.
- **CLAUDE.md 변경 이력 표(append-only markdown table)의 병합 충돌은 "선택"이 아니라 "합치기"다.** charades 병합 실측: `main`(로또 impeccable audit 감사 로그)과 `fun/charades`(charades 빌드+하네스 진화 로그)가 각자 표 끝에 서로 다른 새 행을 추가해 같은 위치에서 conflict — 두 로그 모두 유효한 독립 기록이라 한쪽을 버리면 그 세션의 실제 작업 이력이 사라진다. **해소법:** `<<<<<<<`/`=======`/`>>>>>>>` 마커만 제거하고 **양쪽 행을 전부 남긴다**(순서는 날짜순으로 자연스럽게 재배치, 중복 방지 위해 마커 제거 후 동일 블록이 두 번 나오지 않는지 grep 재확인). 이것은 i18n JSON 형제-객체 인터리빙 병합 함정(수동 재구성 필요)의 **단순 버전**이다 — 여기선 각 행이 완결된 한 줄이라 교차 인터리빙 위험 없이 단순 concat으로 안전하게 해결된다.
- **여러 세션이 같은 로컬 `main` 워크트리를 공유하면, 로컬 `main` 브랜치 ref 자체가 내 명령 사이에 다른 세션에 의해 전진할 수 있다 — "방금 확인했다"는 몇 초 뒤에도 유효하지 않을 수 있다.** cheer-autosize 세션 실제: 로또 세션과 별도로 cheer 도구에 impeccable audit 폴리시를 가하는 **세 번째 세션**이 동시에 같은 저장소를 조작 중이었다. `git fetch`+`git log`로 "local main..origin/main 6커밋 차이"를 확인한 직후 `git merge --ff-only origin/main`을 실행했더니 "이미 업데이트 상태"라 떴다 — 그 찰나에 다른 세션이 이미 로컬 `main`을 그 지점까지 fast-forward해놓은 것. 몇 분 뒤 재확인하니 로컬 `main`이 이번엔 **origin보다 앞서** 있었다(다른 세션이 `fun/cheer` 브랜치를 로컬에서 병합했지만 아직 push 전). **예방:** ① 병합/커밋/푸시 직전마다 매번 새로 `git status --short` + `git log -1 --oneline HEAD` + `git fetch && git log -1 --oneline origin/main`을 다시 실행하고, 이전 턴에서 확인한 상태를 재사용하지 않는다 ② 로컬 HEAD가 예상과 다르면(다른 세션이 이미 병합/커밋했으면) 그 내용을 신뢰하되 **직접 tsc+전체 vitest+빌드로 재검증한 뒤에만** push한다(그 세션의 "주장"이 아니라 내가 재현한 결과로 판단) ③ push는 마지막 순간에 한 번만 시도하고, 실패(non-fast-forward)하면 다시 fetch부터 반복한다 — 몇 초 전 상태를 근거로 강행(force 등)하지 않는다.
...
**세션 자체 컨텍스트가 이미 크면 첫 스폰부터 즉사할 수 있다 — 이때는 재시도보다 리더 직접 구현이
유일한 진행 경로다.** charades 빌드 실측: 리더 세션 컨텍스트(CLAUDE.md 변경 이력 전체 포함)가 이미
매우 큰 상태에서 domain-engineer에게 보낸 (합리적 길이의) 브리프가 작업 착수 전, 브리프 전송
직후 즉시 "Prompt is too long"으로 사망 — 브리프를 줄여도 재현됨. **진단**: 먼저 `Reply with just
"ok"` 같은 최소 브리프로 같은 서브에이전트 타입을 스폰해본다. 그것도 실패하면 "브리프가 길어서"가
아니라 "세션이 전반적으로 막혀서"이므로 서브에이전트 위임을 포기하고 **리더가 Read/Write/Edit/Bash로
직접 구현**한다(이번 빌드는 엔진 이관·도메인·UI·i18n·플랫폼 배선 전부를 이 경로로 완주했다).

## 알고리즘 공유 요청("A와 비슷한 B, 중복 최소화") — 콘텐츠 무관 로직만 추출, UI는 사용자에게 확인

"기존 도구 A와 비슷하니 코드를 공유/재사용하라"는 요청을 받으면, 대상 로직이 진짜 **콘텐츠 무관 순수
함수**인지 먼저 실측(임포트 그래프·타입 결합 확인)한다. charades 실측: speed-quiz의
`shuffle/game-reducer/sound/favorites/slug`는 카테고리 enum 등 도구 어휘에 전혀 결합돼 있지 않아
안전하게 신규 공유 모듈(`src/lib/party-word-game/`)로 추출 가능했다. **추출 방식은 파일 이동이 아니라
위임**: 원 도구(A)의 해당 파일들은 **기존 공개 경로 그대로** `export * from '@/lib/<shared>/<name>'`
재export로 전환 — 그래야 A의 기존 테스트가 무수정으로 통과해야만 회귀 0이 증명된다(수정한 뒤 통과가
아니라 **손대지 않았는데 통과**해야 진짜 증명). `schema`/`merge`/`catalog`처럼 카테고리 enum·기본값이
실제로 다른 파일은 도구별로 유지하고 패턴만 미러링한다. **UI 컴포넌트까지 공유할지는 트레이드오프이며
리더가 임의로 정하지 말고 사용자에게 확인**(AskUserQuestion) — 이미 배포·폴리시를 거친 UI라면
"엔진만 공유, UI는 각자"가 회귀 위험을 크게 낮춘다(구조적 중복은 감수, 알고리즘 중복만 제거).
...
- **에이전트의 *부정* 보고("못 했다/미완/제약으로 못 만듦")도 산출물로 검증한다 — idle≠완료의 대칭.** jwt-decoder 실측: platform-engineer가 최종 보고에서 "`useJwtDecoder` 훅 미완(Write tool constraint)"이라 **명시적으로 못 했다고** 적었으나, 실제로는 `useJwtDecoder.ts`(6.7KB) + `useJwtDecoder.test.ts`(16 tests 그린)가 이미 디스크에 완성돼 있었다(다른 에이전트가 썼거나 자기가 쓰고 세션 말미에 잊음). 그 보고를 믿고 훅을 재지시했으면 중복 작업 + 계약 충돌이었을 것. **규율: 긍정 보고("완료")뿐 아니라 부정 보고("못함/미완")도 `ls`/`grep`/`vitest run <scope>`로 실물을 확인한 뒤에만 재작업 여부를 판정한다.** 긴 세션 말미의 컨텍스트 혼란으로 에이전트는 *이미 한 일을 "못 했다"고 오보고*할 수 있다 — "못 했다"는 "안 돼 있다"를 증명하지 못한다(주장≠증명의 부정형).
...
**새 도구를 스캐폴딩할 때, 이미 존재하는 공유 훅으로 채워야 할 prop을 리터럴로 하드코딩하지 않았는지 확인하라 — 컴파일·테스트는 통과하지만 그 훅이 보장하는 사용자 설정(예: OS 접근성 설정)이 새 도구에서만 무시된다.** charades 실측: 코드베이스에 이미 `@/hooks/useReducedMotion`(cheer·roulette 등 여러 도구가 소비 중)이 있었는데, `Charades.tsx`가 `GameBoard`에 `reducedMotion={false}`를 **리터럴로 고정**해 넘겨 `prefers-reduced-motion` 사용자도 타이머 pulse 애니메이션을 강제로 받았다(tsc·유닛·빌드·E2E 전부 그린 — 하드코딩 리터럴은 타입이 맞으므로 아무 것도 못 잡는다). 예방: 새 도구 컴포넌트를 작성/리뷰할 때 boolean/설정성 prop에 `true`/`false`/매직 상수를 직접 대입하는 지점을 grep(`={false}\|={true}\|={null}` 등 리터럴 대입)하고, 그 prop 이름과 같은 뜻의 공유 훅(`useReducedMotion`·`useLocale`·`useConsent` 등)이 `src/hooks/`에 이미 있는지 먼저 확인한다.
...
## 거대 카탈로그(i18n JSON) 편집은 서브에이전트에 위임하지 말고 리더가 스크립트로 (cheer 세션 교훈)

`src/i18n/messages/{ko,en}.json`은 수천 줄이라 **읽거나 편집하는 서브에이전트가 "Prompt is too long"으로 죽는다**(cheer 세션 실측: platform·ui 에이전트가 i18n 삽입 중 컨텍스트 소진 사망, 사망 전 부분 편집이 **엉뚱한 중첩 위치**[루트에 삽입, `tools` 밖]와 **중복 키**[`presets.concert`가 탭 라벨 문자열과 프리셋 객체 둘 다]를 남김 — JSON은 파스되나 `tools.<slug>` undefined·라벨 소실). **예방:** 새 도구의 `tools.<slug>.*` 블록은 **리더가 Node 스크립트로 결정론적 삽입** — `const j=require(file); j.tools.<slug>={...}; write(JSON.stringify(j,null,2)+'\n')`. 그다음 `git diff --numstat`으로 **additions-only**(삭제≈1=직전 키의 콤마)임을 확인하면 재포맷 churn 0. 서브에이전트에는 "i18n 키를 *소비*하라"만 주고(컴포넌트가 `t()` 호출), **파일 편집은 리더 단독 소유**로 병렬 JSON 충돌도 없앤다.

## architect i18n 계약: 한 키가 값(문자열)과 네임스페이스(객체) 둘 다일 수 없다 (cheer 세션 교훈)

blueprint가 `presets.concert`(탭 라벨 "콘서트")와 `presets.concert.<id>`(프리셋 문구)를 동시에 요구하면 **구조적으로 불가능**하다(JSON 키는 문자열이거나 객체, 둘 다 불가 → 중복 키로 라벨 소실). architect는 그룹/탭 라벨과 그 하위 항목을 **분리 서브트리**로 설계한다: `presets.tabs.<situation>`(라벨) + `presets.phrases.<situation>.<id>`(항목). 컴포넌트도 이 분리 경로를 소비. 병렬로 UI가 blueprint의 충돌 키를 이미 배선했으면 리더가 통합 시 컴포넌트+카탈로그를 함께 분리 경로로 교정한다.
...
## SPEC의 "플랫폼 제공, 재구현 금지" 선언은 실제 배선을 검증해야 한다 (barcode-generator audit 세션)

SPEC이 "Toast 시스템은 플랫폼이 제공하므로 재구현 금지"라고 명시했는데도, 초기 구현은 `showToast`를 `console.log`만 찍는 로컬 스텁으로 만들었다 — 다운로드/복사 성공·실패가 사용자에게 전혀 안 보이는데(스크린리더도 무음) tsc·유닛·빌드는 전부 그린이었고, `/impeccable audit`가 실제 라이브 클릭으로만 적발했다. 실재하는 공유 컴포넌트는 `src/components/ui/Toast.tsx`(`{message,type,onDismiss}` prop + 오케스트레이터가 `toast` state를 소유하고 조건부 렌더 — roulette·new-word 등 다수가 이미 이 패턴으로 소비 중)였는데 안 쓰였다. **더 심각한 건: 비슷한 시기에 만들어진 `qr-code/DownloadButtons.tsx`에도 정확히 같은 패턴의 가짜 `showToast`(no-op, "A platform Toast can be wired here later"라는 동일한 자기고백 주석까지)가 있었다** — 같은 블루프린트를 복제해 만든 형제 도구들이 같은 결함을 공유하고 있었다는 뜻.

**규율:**
- SPEC에 "플랫폼이 제공, 재구현 금지"라고 적힌 항목(Toast·ConsentBanner·AdSlot·ErrorBoundary 등)을 새 도구가 쓸 때는, 착수 전 `src/components/ui/`(또는 해당 계층)에서 실제 공유 컴포넌트를 찾고, 가장 최근에 완성된 형제 도구 하나를 레퍼런스 삼아 그 소비 패턴 그대로 배선한다. "이미 제공됨"을 가정만 하고 로컬 스텁·no-op·console.log로 자리만 채우지 않는다.
- 한 도구에서 "로컬 스텁이 실제 공유 컴포넌트를 대체하고 있다"는 패턴을 발견하면, 비슷한 시기/블루프린트로 만들어진 형제 도구에도 같은 결함이 있는지 그 자리에서 `grep -rn "console.log\|Toast can be wired" src/components/tools/*/`류로 전수 확인한다. 이번 세션은 barcode-generator만 고쳤고 qr-code의 동일 결함 발견은 사용자에게 보고만 하고 수정은 별도 세션으로 미뤘다 — **범위 밖에서 발견한 동일 결함은 조용히 넘기지 말고 반드시 보고한다.**
