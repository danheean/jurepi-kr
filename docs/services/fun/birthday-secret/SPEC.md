...
CRITICAL (external links, no image hosting): Stone/flower **photos** are NOT hosted (no licensed/stock photography of real gemstones or flowers is stored or served). Each stone/flower still shows a "🔍 구글 이미지" link that opens Google Images search in a new tab (`rel="noopener"`), for visitors who want to see the real thing. Extended birth attributes we do NOT author (탄생주/과일/조/목/공룡/별) are covered by a "더 많은 탄생 정보 →" link to `https://otanjoubi.jp/ko/select.php?month=MM&day=DD` (user's date auto-filled, new tab).

The 12 monthly **birthstone illustrations** are the one exception to "no image hosting": they are original, non-photographic flat-vector illustrations generated locally with the `jurepi-image-gen` skill (Ollama, dev-time only — never a real photo, so the copyright concern above does not apply) and committed as static assets at `public/images/birthday-secret/stones/<01-12>.webp` (prompt/seed provenance in `docs/image-prompts/birthday-secret-stones.json`). They render as a decorative thumbnail (`alt=""`, `aria-hidden`) next to the stone name — the Google Images link stays untouched alongside it. Flower/color remain photo-link-only; only the finite, month-keyed stone set gets a generated illustration.
</overview>
